...
export default function Home() {
  ...
  const images = ["/assets/hero/bg 1.jpg", "/assets/hero/bg 2.jpg", "/assets/hero/bg 3.jpg"];
    ...
    <div className="w-full bg-white overflow-hidden text-gray-900">
      ...
      <section className="relative h-screen flex flex-col items-center justify-center overflow-hidden bg-white px-4">
        <div className="absolute inset-0 z-0 overflow-hidden bg-white">
          <AnimatePresence initial={false}>
            <motion.div
              ...
              animate={{ opacity: 0.6, scale: [1.1, 1.05] }}
              exit={{ opacity: 0, scale: 1 }}
              transition={{ duration: 0.5, ease: "easeOut" }}
              className="absolute inset-0 w-full h-full"
            >
              <img src={images[currentIdx]} alt="Background" className="w-full h-full object-cover grayscale" loading="eager" />
        ...
        <div className="absolute inset-0 z-0 flex flex-col justify-center opacity-[0.02] pointer-events-none select-none">
          <HorizontalMarquee speed={3} direction={1} className="text-[10vw] font-black uppercase leading-none text-redAccent">AI & DS • AI & DS • AI & DS •</HorizontalMarquee>
          <HorizontalMarquee speed={2} direction={-1} className="text-[10vw] font-black uppercase leading-none text-gray-900">Pioneering • Future • Pioneering •</HorizontalMarquee>
        </div>
        ...
        <motion.div style={{ y: yHeroText }} className="relative z-10 w-full max-w-5xl mx-auto">
          ...
          <div className="flex flex-col md:flex-row items-end justify-between mt-6 gap-8">
            <motion.p
              style={{ y: yHeroSubtext }} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.5, duration: 1 }}
      ...
      <section className="relative min-h-[60vh] flex flex-col md:flex-row bg-redAccent overflow-hidden border-t-[8px] border-yellow-400">
        <div className="w-full md:w-1/2 h-[400px] md:h-auto relative overflow-hidden">
          <ScrollReveal className="w-full h-full">
            <VariableSpeedImage src={getVal('home_admissions_image', '/rit.jpg')} alt="Admissions" className="w-full h-full object-cover grayscale-[0.5] hover:grayscale-0 transition-all duration-1000" speed={0.05} />
            ...
            {[
              ...
              <ParallaxSection key={i} distance={50}>
                <div className="flex flex-col items-center text-center">
                  <div className="text-6xl font-black text-redAccent leading-none tracking-tighter mb-4">
          ...
          <div className="flex flex-col md:flex-row justify-between items-end mb-12 gap-8">
            <SkewScroll>
              <h2 className="text-4xl md:text-6xl font-black text-gray-900 leading-[1] uppercase tracking-tighter">
                Innovation <br /><span className="text-redAccent">Partners</span>
              </h2>
            </SkewScroll>
                    ...
                    <div className="p-6 flex-grow text-center md:text-left">
                      ...
                      <div className="flex flex-wrap gap-2 justify-center md:justify-start">
                        {partner.tags.map((tag, j) => (
                          <span key={j} className="bg-redAccent/10 text-redAccent border border-redAccent/20 text-[9px] font-black px-3 py-1 rounded-full uppercase tracking-widest">{tag}</span>
                        ))}
                      </div>
                    </div>
                    {partner.image && (
