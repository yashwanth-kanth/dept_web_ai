...
import { Calendar, MapPin, Sparkles, Zap, Award, Users } from 'lucide-react';
...
export default function Events() {
  ...
  const featuredEvents = [
    {
      title: "TECH-VISHVA 2024",
      description: "Our annual flagship technical symposium bringing together the brightest minds in AI & DS from across the country for knowledge exchange and competition.",
      image: "/assets/events/symposium.png",
      tag: "SYMPOSIUM",
      color: "text-red-500"
    },
    {
      title: "AI DEEP DIVE WORKSHOPS",
      description: "Hands-on sessions led by industry experts from DigiSpot and Icanio, focusing on Generative AI, MLOps, and real-world data engineering.",
    ...
    {
      title: "D-HACK 3.0",
      description: "A 36-hour intensive hackathon where students build innovative solutions for global challenges using Artificial Intelligence and Cloud computing.",
            ...
            {featuredEvents.map((event, i) => (
              <div key={i} className={`flex flex-col ${i % 2 === 1 ? 'md:flex-row-reverse' : 'md:flex-row'} items-center gap-16`}>
                ...
                <div className="flex-1 space-y-6">
                  <ScrollReveal>
                    <span className={`text-[10px] font-black tracking-[0.2em] px-3 py-1 bg-white border border-gray-100 rounded-full shadow-sm ${event.color}`}>
                      {event.tag}
                    </span>
                    ...
                    <div className="pt-6">
                      <Magnetic distance={0.2}>
                        <button className="text-xs font-black uppercase tracking-widest text-redAccent flex items-center gap-2 group">
                    ...
                    <MouseParallax factor={10} className="h-full">
                      <div className="bg-gray-50 rounded-[2rem] overflow-hidden border border-gray-100 flex flex-col h-full group hover:border-redAccent/20 hover:shadow-2xl transition-all duration-500">
                        <div className="relative h-56 overflow-hidden">
                          {event.image ? (
                            <img src={event.image} alt={event.title} className="w-full h-full object-cover grayscale group-hover:grayscale-0 group-hover:scale-110 transition-all duration-700" />
                          ) : (
