...
export default function AdminDashboard() {
  ...
  if (isPending) {
    return <div className="min-h-screen flex items-center justify-center font-black text-gray-400 uppercase tracking-widest">Loading...</div>;
  }
      ...
      <aside className="w-64 bg-white border-r border-gray-200 flex flex-col fixed h-full z-20">
            ...
            {tabs.map(tab => (
              <button
                ...
                className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-bold uppercase tracking-tight transition-all ${
                  activeTab === tab.id 
                    ? 'bg-redAccent text-white shadow-lg shadow-redAccent/20' 
        ...
        <div className="mt-auto p-6 border-t border-gray-100">
          <button
            ...
            className="w-full flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-bold uppercase tracking-tight text-red-500 hover:bg-red-50 transition-all"
          >
            <LogOut size={18} />
            Sign Out
          </button>
        ...
        <header className="mb-10 flex justify-between items-end">
          <div className="animate-in fade-in slide-in-from-left-4 duration-700">
            ...
            <p className="text-gray-500 font-bold uppercase text-[10px] tracking-widest">
              Administrator Control Panel • {session.user.name}
            </p>
          </div>
          <Link 
            to="/" 
            className="flex items-center gap-2 px-6 py-2 border-2 border-gray-200 rounded-full text-xs font-black uppercase tracking-widest text-gray-500 hover:border-redAccent hover:text-redAccent transition-all bg-white shadow-sm"
          >
    ...
    <div className="space-y-8">
       <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="p-8 bg-gray-50 rounded-3xl border border-gray-100">
             <h3 className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-4">Security</h3>
             <p className="text-2xl font-black text-gray-900 leading-none">Vetted Admin</p>
          </div>
          <div className="p-8 bg-gray-50 rounded-3xl border border-gray-100">
             <h3 className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-4">Content Power</h3>
             <p className="text-2xl font-black text-gray-900 leading-none">Advanced CMS</p>
          </div>
          <div className="p-8 bg-gray-50 rounded-3xl border border-gray-100">
             <h3 className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-4">Media Storage</h3>
             <p className="text-2xl font-black text-gray-900 leading-none">Dynamic Uploads</p>
          </div>
       </div>
...
function ConfigTab() {
  ...
  const updateConfig = useMutation(api.siteConfig.updateConfig);
  ...
  const handleFileUpload = async (event, key) => {
    ...
    try {
      ...
      if (!result.ok) throw new Error(resData.message || "Upload failed");
      
      const { storageId } = resData;
      await updateConfig({ key, value: storageId });
    } catch (err) {
  ...
  const defaultKeys = [
    ...
    { key: 'footer_location_extra', label: 'Location Details (City/State)', page: 'Footer', type: 'text' },
             ...
             {pages.map(p => (
               <button 
                 ...
                 className={`px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${
                   selectedPage === p 
                     ? 'bg-redAccent text-white shadow-lg shadow-redAccent/20' 
                     : 'bg-white text-gray-500 hover:text-gray-900 border border-gray-100 hover:border-gray-300'
                 }`}
               >
                 {p}
               </button>
                    ...
                    <div className="animate-in fade-in zoom-in-95">
                      {item.type === 'textarea' || isCode ? (
                        <textarea 
                          ...
                          placeholder={isCode ? "/* Enter global CSS overrides here */" : ""}
                          className={`w-full bg-white border border-gray-200 p-4 rounded-2xl font-bold text-gray-900 outline-none focus:ring-2 focus:ring-redAccent/20 min-h-[120px] ${isCode ? 'font-mono text-sm' : ''}`}
                          autoFocus
                        />
                      ) : item.type === 'color' ? (
                        <div className="flex items-center gap-4 bg-white border border-gray-200 p-4 rounded-2xl">
                          ...
                          <input 
                            ...
                            value={val}
                            onChange={e => setVal(e.target.value)}
                            className="flex-1 font-mono font-bold text-gray-900 outline-none"
                            placeholder="#000000"
                          />
                        </div>
                      ) : (
                        <input 
                          value={val} 
                          onChange={e => setVal(e.target.value)}
                          className="w-full bg-white border border-gray-200 p-4 rounded-2xl font-bold text-gray-900 outline-none focus:ring-2 focus:ring-redAccent/20"
                          autoFocus
                        />
                      )}
                       ...
                       {isImage ? (
                         <div className="flex items-center gap-4">
                            <div className="w-20 h-20 bg-gray-100 rounded-2xl border-2 border-white shadow-inner overflow-hidden">
                               {config?.value ? (
                                 <img src={resolveUrl(config.value)} className="w-full h-full object-cover" />
                               ) : (
                                 <div className="w-full h-full flex items-center justify-center"><ImageIcon className="text-gray-200" /></div>
                               )}
                            </div>
                            <div>
                               <p className="text-[10px] font-black text-gray-300 uppercase truncate max-w-[200px]">{config?.value || 'No file selected'}</p>
                               <div className="flex gap-2 mt-2">
                                  ...
                                  <button onClick={() => { setEditingKey(item.key); setVal(config?.value || ''); }} className="px-4 py-2 text-[9px] font-black uppercase text-gray-400 hover:text-gray-900 tracking-widest">Manually Set ID</button>
                               </div>
                            </div>
                         </div>
                       ) : isCode ? (
...
function EventsTab() {
  ...
  const [formData, setFormData] = useState({
    title: '', date: '', venue: '', description: '', status: 'upcoming', isFeatured: false, rsvps_count: 0, image: ''
  });
  ...
  const toggleFeatured = async (event) => {
     ...
     await updateEvent({ ...rest, id: _id, isFeatured: !event.isFeatured });
  };
  ...
  return (
    <div className="space-y-6">
         ...
         <form onSubmit={handleAdd} className="bg-gray-50 p-10 rounded-[3rem] border border-gray-100 grid grid-cols-1 md:grid-cols-2 gap-8 animate-in zoom-in-95 duration-300 shadow-sm">
            ...
            <div className="md:col-span-2">
               ...
               <div className="flex items-center gap-6 p-5 bg-white rounded-2xl border border-gray-100">
                  {formData.image && <img src={formData.image.startsWith('http') ? formData.image : `${import.meta.env.VITE_CONVEX_SITE_URL}/api/storage/${formData.image}`} className="w-20 h-20 rounded-xl object-cover" />}
       ...
       <div className="grid gap-4 mt-10">
          {events.map(event => (
            <div key={event._id} className="p-6 bg-gray-50 rounded-3xl border border-gray-100 flex items-center justify-between group">
               <div className="flex items-center gap-6">
                 {event.image ? (
                   <img src={event.image.startsWith('http') ? event.image : `${import.meta.env.VITE_CONVEX_SITE_URL}/api/storage/${event.image}`} className="w-16 h-16 rounded-2xl object-cover grayscale brightness-95" />
                 ) : (
               ...
               <div className="flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                  <button onClick={() => toggleFeatured(event)} className={`p-3 rounded-xl transition-all ${event.isFeatured ? 'text-yellow-500 bg-white shadow-sm' : 'text-gray-300 hover:bg-white'}`}>
                    <Plus size={18} />
                  </button>
                  <button onClick={async () => await deleteEvent({ id: event._id })} className="p-3 bg-white text-gray-300 hover:text-redAccent rounded-xl shadow-sm transition-all">
                    <Trash2 size={18} />
                  </button>
               </div>
            </div>
...
function PartnersTab() {
  ...
  const handlePartnerUpload = async (e, type) => {
     ...
     try {
       ...
       const res = await fetch(uploadUrl, { method: "POST", headers: { "Content-Type": file.type }, body: file });
       const { storageId } = await res.json();
       setFormData(prev => ({ ...prev, [type]: storageId }));
     } catch (err) { alert(err.message); }
  ...
  return (
    <div className="space-y-6">
       ...
       <div className="grid gap-4 mt-6">
          {partners.map(p => (
            <div key={p._id} className="p-6 bg-white rounded-3xl border border-gray-100 flex items-center justify-between group shadow-sm">
               <div className="flex items-center gap-6">
                 <img src={p.logo.startsWith('http') ? p.logo : `${import.meta.env.VITE_CONVEX_SITE_URL}/api/storage/${p.logo}`} className="w-12 h-12 object-contain grayscale" />
                 <div>
               ...
               <button onClick={async () => await deletePartner({ id: p._id })} className="p-3 text-gray-300 hover:text-redAccent transition-all opacity-0 group-hover:opacity-100">
                 <Trash2 size={18} />
               </button>
            </div>
          ))}
...
function UsersTab() {
  ...
  const handleCreateUser = async (e) => {
    ...
    try {
      ...
      setSuccessMsg(`Account Validated: ${name}`);
  ...
  return (
    <div className="max-w-2xl px-4 py-8">
       <div className="flex items-center gap-4 mb-10">
          <div className="p-4 bg-gray-900 rounded-3xl text-white shadow-xl shadow-gray-900/40">
             <UsersIcon size={32} />
          </div>
          <div>
       ...
       {successMsg && (
         <div className="mb-8 p-6 bg-green-50 border border-green-100 text-green-700 rounded-3xl flex items-center gap-4 animate-in slide-in-from-top-4">
            <CheckCircle size={24} />
            <span className="font-black uppercase text-xs tracking-widest">{successMsg}</span>
         </div>
       )}
       ...
       <form onSubmit={handleCreateUser} className="space-y-6 bg-gray-50 p-10 rounded-[3rem] border border-gray-100">
         ...
         <div>
           <label className="block text-[10px] font-black text-gray-400 uppercase tracking-widest mb-2 ml-3">Secure Email (Internal)</label>
