...
export default function Navbar() {
  ...
  return (
    <nav className="bg-white/90 backdrop-blur-md fixed w-full z-50 border-b border-gray-200 shadow-sm">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
          ...
          <div className="-mr-2 flex md:hidden">
            <button onClick={() => setIsOpen(!isOpen)} className="text-gray-600 hover:text-redAccent inline-flex items-center justify-center p-2 rounded-md focus:outline-none">
      ...
      {isOpen && (
        <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="md:hidden bg-white border-b border-gray-200 shadow-md">
          <div className="px-2 pt-2 pb-3 space-y-1 sm:px-3">
            {navLinks.map((link) => (
              <Link key={link.name} to={link.path} onClick={() => setIsOpen(false)} className="text-gray-800 hover:text-redAccent block px-3 py-2 rounded-md text-base font-bold">
                {link.name}
              </Link>
            ))}
            {!isPending && session ? (
              <>
                <Link to="/admin/dashboard" onClick={() => setIsOpen(false)} className="text-gray-800 hover:text-redAccent block px-3 py-2 rounded-md text-base font-bold">Admin Dashboard</Link>
                <button onClick={async () => { await signOut(); setIsOpen(false); }} className="text-gray-800 hover:text-redAccent block w-full text-left px-3 py-2 rounded-md text-base font-bold border-t border-gray-100 mt-2 pt-2">
                  Sign Out ({session.user.name})
                </button>
              </>
            ) : (
              <Link to="/admin" onClick={() => setIsOpen(false)} className="text-redAccent block px-3 py-2 rounded-md text-base font-bold border-t border-gray-100 mt-2">Admin Login</Link>
            )}
          </div>
        </motion.div>
      )}
