          ...
          <div className="flex-shrink-0 flex items-center gap-3">
            <Magnetic distance={0.2}>
              <Link to="/" className="block">
                <img src="/rit-logo.png" alt="RIT Logo" className="h-[46px] w-auto object-contain drop-shadow-sm" />
              </Link>
            </Magnetic>
            <div className="hidden sm:flex flex-col border-l-2 border-gray-300 pl-3">
              <span className="text-[17px] font-black text-gray-900 leading-tight uppercase tracking-tight">Ramco Institute of Technology</span>
              <span className="text-[13px] font-black text-gray-700">An Autonomus Institution, Rajapalayam </span>
            </div>
          ...
          <div className="hidden md:flex items-center">
            {isPending ? (
              ...
              <div className="flex items-center gap-3">
                <span className="text-sm font-bold text-gray-700 hidden lg:block">Hi, {session.user.name.split(' ')[0]}</span>
                <Link to="/admin/dashboard" className="text-sm font-bold text-gray-700 hover:text-redAccent mx-2 hidden lg:block">Dashboard</Link>
                <button
                  onClick={async () => await signOut()}
                  className="bg-gray-100 text-gray-700 hover:bg-redAccent hover:text-white px-5 py-2 rounded-full text-sm font-bold transition"
                >
                  Sign Out
                </button>
              </div>
